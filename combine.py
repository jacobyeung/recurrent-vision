    ...
    for num_recurrence in range(3):
        ...
        for num_channels in [8, 16, 24]:
            with open(
                ...
                data = pickle.load(f)
            print(
                f"CIFAR_num_layers={num_layers}, num_recurrence={num_recurrence}, num_channels={num_channels}, test_acc={data['test_acc']}, test_noisy_acc={data['test_noisy_acc']}, test_masked_acc={data['test_masked_acc']}"
            )
...
with open(f"./CIFAR_combined.pkl", "wb") as f:
    pickle.dump(combined, f)

with open(f"./CIFAR_combined_list.pkl", "wb") as f:
